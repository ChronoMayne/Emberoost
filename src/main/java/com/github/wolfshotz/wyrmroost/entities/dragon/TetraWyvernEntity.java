...
public class TetraWyvernEntity extends TameableDragonEntity implements IAnimatable
{
    private static final EntitySerializer<TetraWyvernEntity> SERIALIZER = TameableDragonEntity.SERIALIZER.concat(b -> b
            ...
            .track(EntitySerializer.INT, "KnockOutTime", TetraWyvernEntity::getKnockOutTime, TetraWyvernEntity::setKnockoutTime));

    public static final int ARMOR_SLOT = 0;
    private static final int MAX_KNOCKOUT_TIME = 1800; // 1.5 minutes

    public static final AnimationBuilder TETRA_ROAR_ANIMATION = new AnimationBuilder().addAnimation("tetrawyvern.animation.roar", false);
    public static final AnimationBuilder TETRA_SLEEP_ANIMATION = new AnimationBuilder().addAnimation("tetrawyvern.animation.sleep", true);
    public static final AnimationBuilder TETRA_BITE_ANIMATION = new AnimationBuilder().addAnimation("tetrawyvern.animation.bite", false);

    public static final DataParameter<Boolean> KNOCKED_OUT = EntityDataManager.defineId(TetraWyvernEntity.class, DataSerializers.BOOLEAN);
    ...
    public TetraWyvernEntity(EntityType<? extends TameableDragonEntity> dragon, World level)
    {
        super(dragon, level);
    ...
    @Override
    public EntitySerializer<TetraWyvernEntity> getSerializer()
    {
        return SERIALIZER;
    }
    ...
    public void aiStep()
    {
        ...
        if (!level.isClientSide)
        {
            ...
            if (noAnimations() && !isKnockedOut() && !isSleeping() && isJuvenile() && getRandom().nextDouble() < 0.0004)
    ...
    private void setSleepingDragon(boolean b) {
        entityData.define(SLEEPING, b);
    }

    private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event){
        if (isKnockedOut() || getEntity() == null){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("TETRA_SLEEP_ANIMATION", true));
        }
    ...
    {
        if (!level.isClientSide){
             setAttackingDragon(TETRA_BITE_ANIMATION, true);}
    }

    private void setAttackingDragon(AnimationBuilder TETRA_BITE_ANIMATION, boolean b) {
    }
    ...
    public static AttributeModifierMap.MutableAttribute getAttributeMap()
    {
        ...
        return MobEntity.createMobAttributes()
                .add(MAX_HEALTH, 130 + 50)
                .add(MOVEMENT_SPEED, 0.22 - 5)
                .add(KNOCKBACK_RESISTANCE, 1)
                .add(FOLLOW_RANGE, 60 + 20)
                .add(ATTACK_KNOCKBACK, 4)
                .add(ATTACK_DAMAGE, 12)
                .add(FLYING_SPEED, 0.121 + 0.05);
    }
    ...
    class AttackGoal extends Goal
    {
        ...
        public void tick()
        {
            ...
            else if (distFromTarget <= 24 && noAnimations() && canSeeTarget)
            {
                yBodyRot = yRot = (float) Mafs.getAngle(TetraWyvernEntity.this, target) + 90;
                meleeAttack();
            }
