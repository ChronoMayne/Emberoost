...
public class RoyalRedEntity extends TameableDragonEntity implements IAnimatable {
    ...
    private static <T> T getKnockOutTime(RoyalRedEntity royalRedEntity) {}

    public static final int ARMOR_SLOT = 0;
    private static final int MAX_KNOCKOUT_TIME = 3200; // 3 minutes

    public static final AnimationBuilder RoyalRed_ROAR_ANIMATION = new AnimationBuilder().addAnimation("royalred.animation.roar", false);
    public static final AnimationBuilder RoyalRed_SLEEP_ANIMATION = new AnimationBuilder().addAnimation("royalred.animation.sleep", true);
    public static final AnimationBuilder RoyalRed_BITE_ANIMATION = new AnimationBuilder().addAnimation("royalred.animation.bite", false);
    ...
    public void aiStep() {
        ...
        if (!level.isClientSide) {
            ...
            if (noAnimations() && !isKnockedOut() && !isSleeping() && !isBreathingFire() && isJuvenile() && getRandom().nextDouble() < 0.0004)
                AnimationPacket.send(this, RoyalRed_ROAR_ANIMATION);
    ...
    private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
        if (isKnockedOut() || getEntity() == null) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("RoyalRed_SLEEP_ANIMATION", true));
        }
    ...
    public void recievePassengerKeybind(int key, int mods, boolean pressed) {
        ...
        if (key == KeybindHandler.MOUNT_KEY && pressed && !isBreathingFire()) {
            if ((mods & GLFW.GLFW_MOD_CONTROL) != 0) setAnimation();
            else meleeAttack();
        }
    ...
    public void meleeAttack() {
        if (!level.isClientSide) {
            setAttackingRoyalRed(RoyalRed_BITE_ANIMATION, true);
        }
    }

    private void setAttackingRoyalRed(AnimationBuilder RoyalRed_BITE_ANIMATION, boolean b) {
    }
    ...
    public static AttributeModifierMap.MutableAttribute getAttributeMap ()
    {
        ...
        return MobEntity.createMobAttributes()
                ...
                .add(FLYING_SPEED, 0.121 + 0.0)
                .add(WREntities.Attributes.PROJECTILE_DAMAGE.get(), 4);
    }

    public void setBreathingFire(boolean b) {
        new AnimationBuilder().addAnimation("royalred.animation.roar", false);
        if (!level.isClientSide);
